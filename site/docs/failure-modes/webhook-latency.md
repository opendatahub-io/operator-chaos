...
|-------|------|----------|---------|-------------|
| `resources` | `string` | Yes | - | Comma-separated Kubernetes resource types to intercept (e.g., 'deployments', 'services') |
| `apiGroups` | `string` | Yes | - | Comma-separated API groups (e.g., 'apps', '' for core) |
| `delay` | `duration` | No | `25s` | Latency to add per request (range: 1s-29s) |
...
|-----------|------------|--------|-------------|
| odh-model-controller | odh-model-controller-webhook-latency | high | Deploying a slow admission webhook (25s delay, just under the 30s API server tim... |
