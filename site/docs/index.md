...
| **Fault types** | [13 types](architecture/injection-engine.md) across 4 categories: pod/network lifecycle, webhook/RBAC/config drift, resource ownership, controller-runtime client faults | Infrastructure: node kill, network chaos, etcd split, zone outage, CPU/memory hog | Mixed: pod kill, node drain, disk fill, HTTP chaos, cloud provider faults | Kubernetes-native: pod kill, network delay/loss, IO stress, time skew, JVM faults |
