...
type: LabelStomping
danger: Medium
description: Modifies or removes labels on operator-managed resources to test label-based reconciliation.
spec_fields:
  - name: apiVersion
    type: string
    required: true
  ...
  - name: labelKey
    type: string
    required: true
    description: Label key to modify or delete
  - name: action
    type: string
    required: true
    description: "Action to perform: 'overwrite' (set a new value) or 'delete' (remove the label)"
  - name: newValue
    type: string
    required: false
    default: "chaos-stomped"
    description: Value to set when action is 'overwrite'
---
