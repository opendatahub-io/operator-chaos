...
func TestChaosTransport_ChaosConfigExcluded(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "http://localhost/api/v1/namespaces/default/configmaps/"+ChaosConfigMapName, nil)
	resp, err := rt.RoundTrip(req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestChaosTransport_ChaosConfigExcludedOnWatch(t *testing.T) {
	fc := NewFaultConfig(map[Operation]FaultSpec{
		OpList: {ErrorRate: 1.0, Error: "chaos list"},
	})
	...
	req := httptest.NewRequest("GET", "http://localhost/api/v1/namespaces/default/configmaps/"+ChaosConfigMapName+"?watch=true", nil)
	resp, err := rt.RoundTrip(req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !inner.called {
		t.Fatal("chaos-config watch should bypass fault injection")
	}
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200, got %d", resp.StatusCode)
	}
...
func TestChaosTransport_WatchMapsToOpList(t *testing.T) {
	fc := NewFaultConfig(map[Operation]FaultSpec{
		OpList: {ErrorRate: 1.0, Error: "list fault"},
	})
	...
	req := httptest.NewRequest("GET", "http://localhost/api/v1/pods?watch=true", nil)
	resp, err := rt.RoundTrip(req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if inner.called {
		t.Fatal("watch request should map to OpList and be fault-injected")
	}
	if resp.StatusCode != http.StatusServiceUnavailable {
		t.Fatalf("expected 503 for watch/list fault, got %d", resp.StatusCode)
	}
}
