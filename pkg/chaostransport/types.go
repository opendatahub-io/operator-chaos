...
// Delay is only applied when the fault activates (error rate check passes first).
...
// MaybeInjectWithContext is like MaybeInject but respects context cancellation
// during delay. Use this on hot paths (HTTP transport, reconcile loops).
func (f *FaultConfig) MaybeInjectWithContext(ctx context.Context, operation Operation) error {
	if f == nil {
		return nil
	}
	f.mu.RLock()
