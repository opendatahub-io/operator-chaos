...
func TestActionInterceptor_FailAfterPreservesActionError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "action error") {
		t.Fatalf("should contain original action error, got: %v", err)
	}
	if !strings.Contains(err.Error(), "chaos after") {
		t.Fatalf("should contain chaos error, got: %v", err)
	}
}

func TestActionInterceptor_FailAfterActionSucceeds(t *testing.T) {
	ai := NewActionInterceptor(map[string]ActionFaultConfig{
		"deploy": {FailAfter: "chaos after"},
	})
	...
	err := fn(context.Background(), nil)
	if err == nil {
		t.Fatal("expected chaos error even when action succeeds")
	}
	var ce *ChaosError
	if !errors.As(err, &ce) {
		t.Fatal("should contain ChaosError")
	}
}
